    ...
    internal class Utils
    {
        public static Solution calculateMetrics(List<Tour> tours, Instance instance)
        {
            ...
            foreach (var tour in tours)
            {
                ...
                totalCrewUsageCost += crew.baseCost;
                grandTotal += sum;
            }
        ...
        public static Solutionv2 calculateMetricsv2(Solutionv2 solution, Instance instance)
        {
            List<Location> GTR = solution.GTR;
            List<Crew> Crews = solution.Crews;
            double totalPenalty = 0.0;
            double totalDrivingCost = 0.0;
            double totalAfterHoursCost = 0.0;
            double totalCrewUsageCost = 0.0;
            ...
            var routeNumber = 0;
            var crew = Crews[routeNumber];
            crew.WorkTime = crew.WorkingTimeWindow.startTime;
            Location prevLocation = null;
            double currentLoad = 0.0;
            foreach (var loc in GTR)
            {
                var penalty = 0.0;
                if (loc.Id !=0)
                {
                    var dist = instance.DistanceMatrix[prevLocation.Id, loc.Id];
                    totalPenalty += Math.Max(0, loc.TimeWindow.Start - (crew.WorkTime + dist)) * instance.toEarlyPenaltyMultiplier;
                    totalPenalty += Math.Max(0, (crew.WorkTime + dist + loc.ServiceTime * crew.serviceTimeMultiplier) - loc.TimeWindow.End) * instance.toLatePenaltyMultiplier;
                    crew.WorkTime += loc.ServiceTime * crew.serviceTimeMultiplier + dist;
            ...
            grandTotal = totalPenalty + totalDrivingCost + totalAfterHoursCost + totalCrewUsageCost;

            return new Solutionv2(GTR, Crews, totalPenalty, totalDrivingCost, totalAfterHoursCost, totalCrewUsageCost, grandTotal);

        }
        public static Solutionv2 generateGreedySolutionv2(Instance instance) //wprowadzic czekanie jezeli sie oplaca
        {
            var greedyGTR = createGreedyGTRv2(instance);
            return new Solutionv2(greedyGTR, instance.Crews);
        }
        private static List<Location> createGreedyGTRv2(Instance instance)
        {
            double[,] distanceMatrix = instance.DistanceMatrix;
            List<Location> locations = instance.Locations;
            List<Crew> Crews = instance.Crews;
